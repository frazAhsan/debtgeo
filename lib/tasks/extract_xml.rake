  ...
  task :extract_xml => :environment do
    ...
    cc_agencies.each do |cc_agency|
        ...
        states.each do |state_css|
            ...
            state = State.create(state_code: state_code.to_s.gsub("\t", ""), name: state_name.to_s.gsub("\t", "")) if state.nil?
            ...
            judicial_districts.each do |jd|
                ...
                judicial_district = JudicialDistrict.create(state_id: state.id, short_name:judicial_district_short_name.to_s.gsub("\t", ""), name: judicial_district_formal_name.to_s.gsub("\t", "")) if judicial_district.nil?
                ...
                agencies_css.each do |ag|
                    ...
                    if agency.nil?
                        organization_name = ag.css("organization_name").text.to_s.gsub("\t", "")
                        address = ag.css("agency_name_address").text.to_s.gsub("\t", "")
                        website = ag.css("agency_web_site_url").text.to_s.gsub("\t", "")
                        ...
                        service_method_css.each do |smc|
                            ...
                            service_method_name = smc.css("service_method_name").text.to_s.gsub("\t", "")
                            phone_number = smc.css("phone_number").text.to_s.gsub("\t", "")
                            phone_extension = smc.css("phone_extension").text.to_s.gsub("\t", "")
                            web_site_url = smc.css("web_site_url").text.to_s.gsub("\t", "")
                            location_address_street_address1 = smc.css("location_address_street_address1").text.to_s.gsub("\t", "")
                            location_address_street_address2 = smc.css("location_address_street_address2").text.to_s.gsub("\t", "")
                            location_address_city = smc.css("location_address_city").text.to_s.gsub("\t", "")
                            location_address_state_code = smc.css("location_address_state_code").text.to_s.gsub("\t", "")
                            location_address_postal_code = smc.css("location_address_postal_code").text.to_s.gsub("\t", "")
                            location_address_phone_number = smc.css("location_address_phone_number").text.to_s.gsub("\t", "")
                            doing_business_as = smc.css("doing_business_as").text.to_s.gsub("\t", "")
                            location_language_name = smc.css("location_language_name").text.to_s.gsub("\t", "")
                            translator_only_flag = smc.css("translator_only_flag").text.to_s.gsub("\t", "")
                            written_material_only_flag = smc.css("written_material_only_flag").text.to_s.gsub("\t", "")
                            full_service_flag = smc.css("full_service_flag").text.to_s.gsub("\t", "")
