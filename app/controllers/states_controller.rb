class StatesController < ApplicationController
  ...
  def zip_codes
    ...
    set_default_values
    @agencies = Agency.where("zip_code = ?", params[:zip_code]).page(params[:page]).per(10).order(:organisation_name)
    render :show
  end
  ...
  def cities
    @state = State.find_by(slug: params[:state_id])
    set_default_values
    @agencies = Agency.where("city = ?", params[:city]).page(params[:page]).per(10).order(:organisation_name)
    render :show
  end
